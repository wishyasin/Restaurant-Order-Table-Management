...
app.post("/api/orders/payment", async (req, res) => {
	...
	console.log(`Ödeme İsteği -> Sipariş: ${orderId}, Masa: ${tableId}, Durum: ${status}`);
	...
	try {
		...
		res.status(200).json({ message: "Masa başarıyla boşaltıldı." });
	} catch (err) {
		console.error("Ödeme İşlem Hatası:", err.message);
		res.status(500).json({ error: err.message });
	}
});
